...
export const TUI_COMMANDS = [
  ...
  { name: '/new', description: 'start a new session' }, { name: '/sessions', description: 'list sessions' },
...
async function handleCommand(line: string, ctx: CommandContext): Promise<void> {
  ...
  if (line === '/sessions') { if (!ctx.config.session.enabled) { ctx.appendSystem('Sessions are disabled.'); return; } const sessions = await new JsonlSessionStore(`${ctx.config.workspaceRoot}/.nju-agent`).list(); ctx.appendSystem(sessions.length ? sessions.map((item) => item.id).join('\n') : 'No sessions.'); return; }
