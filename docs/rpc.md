...
`status` 和 `session/state` 返回的运行状态包含 MCP 快照：已配置 server、Trust/启用状态、连接健康、协议/版本、工具目录哈希和 reload 状态。未 Trust 的工作区会列出配置但不会启动 MCP 子进程。`/reload` 会同时刷新用户插件和 MCP 配置，但只在下一次运行前生效；活动运行不会被热替换。
...
`prompt` 运行期间，服务会发送运行开始、文本增量、推理增量、工具调用、工具结果、消息完成、运行完成或错误事件。工具结果中的 MCP 工具使用 `mcp__<server>__<tool>` 名称，并继续经过主机 ToolExecutor 的 schema、policy、超时、取消、脱敏和审计。取消只影响对应运行；MCP 请求取消或超时会关闭对应外部会话。关闭服务时会等待活动运行收尾，再结束输入输出。
