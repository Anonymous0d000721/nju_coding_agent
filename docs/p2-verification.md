...
| `npm test -- --run` | 通过，38 个测试文件、200 个测试 |
...
| `npm run benchmark` | 通过，所有 benchmark checks 为 `true`；任一 check 失败时脚本返回非零退出码 |
...
- 首 token：`0.022 ms`
- 8 个工具、并发上限 4：`28.455 ms`
- 工具平均耗时：`14 ms`
- 300 条历史 deterministic compaction：`3.71 ms`
- 重试等待：`26.439 ms`
- 12 个插件加载：`2364.39 ms`
- 10 个 MCP server 连接：`1.173 ms`
...
- P2 `4.1`、`4.2`、`4.3`、`4.4` 的全部 checkbox 已与源码、测试、文档和独立提交对应；本次读取保护修复后全量测试为 38 个测试文件、200 个测试。
